...
import com.mincom.ellipse.edoi.ejb.msf340.MSF340_SET_DEFKey;
import com.mincom.ellipse.edoi.ejb.msf340.MSF340_SET_DEFRec;
import com.mincom.ellipse.edoi.ejb.msf341.MSF341Key;
import com.mincom.ellipse.edoi.ejb.msf341.MSF341Rec;
...
import com.mincom.ellipse.edoi.ejb.msf650.MSF650Key;
...
public class ProcessTrb35x extends SuperBatch {
    ...
    private void processRequest() {
        ...
        if(batchParams.productiveUnit?.trim()) {
            ...
            if(prodUnitDto) {
                ...
                fittedEquipmentList.addAll(browseFittedEquipments(equipProdUnit))
            }
        }
    ...
    /**
     * Browse fitted equipment number recursively.
    ...
    private ArrayList<String> browseFittedEquipments(String parentEquipNo) {
        info("browseFittedEquipments ${parentEquipNo}")
        ...
        List results = edoi.search(query).getResults()
        if(!results.isEmpty()) {
            results.each {
                fitEquip.add(it)
    ...
    private void getCondMonitorMeasurementType() {
        ...
        Constraint cMonCode = MSF341Key.compModData.like(batchParams.componentCode + "%")
    ...
    private void fetchCondMonitorMeasurementRecords(QueryImpl query) {
         ...
         * Since there is no service call for MSF341 (Condition Monitor Measurement Defn), use edoi to browse
         */
        edoi.search(query) { MSF341Rec msf341Rec->
    ...
    private boolean retrieveOldCondMonitorMeasurementSet() {
        ...
        try {
            ...
            edoi.search(query) {MSF600Rec msf600Rec->
                msf600Count++
    ...
    private void getFitEquip() {
        ...
        QueryImpl query = new QueryImpl(MSF650Rec.class).and(cFitEquipNo).and(cTracingAccount).and(cRevEtDate)
        try {
            edoi.search(query) {MSF650Rec msf650Rec->
    ...
    private void readCondMonMeas() {
        ...
        QueryImpl query = new QueryImpl(MSF345Rec.class).and(cRecType).and(cEquipNo).and(cCompPosData).and(cMonMeas)

        edoi.search(query) {MSF345Rec msf345Rec->
