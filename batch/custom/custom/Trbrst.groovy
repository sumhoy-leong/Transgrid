...
public class ProcessTrbcmu extends SuperBatch {
    ...
    private static final String SERVICE_NAME_WORKORDER       = "WORKORDER"
    ...
    /**
     * Delete Cond Mon record using service call.
     * @param cmRec Cond Mon record
     * @return CondMeasurementServiceDeleteReplyDTO
     */
    private CondMeasurementServiceDeleteReplyDTO deleteCondMon(CMRecord cmRec) {
        info("deleteCondMon")
        CondMeasurementServiceDeleteReplyDTO returnDTO = null
        try {
            CondMeasurementServiceDeleteRequiredAttributesDTO reqAttr =
                    new CondMeasurementServiceDeleteRequiredAttributesDTO()
            reqAttr.setReturnEquipmentRef(true)
            reqAttr.setReturnCompCode(true)
            reqAttr.setReturnCondMonPos(true)
            reqAttr.setReturnCondMonType(true)
            ...
            CondMeasurementServiceDeleteRequestDTO cmDeleteDTO =
                    new CondMeasurementServiceDeleteRequestDTO()
            cmDeleteDTO.setEquipmentRef(cmRec.equipmentRef)
            cmDeleteDTO.setCompCode(cmRec.compCode)
            cmDeleteDTO.setCondMonPos(cmRec.condMonPos)
            cmDeleteDTO.setCondMonType(cmRec.condMonType)
            cmDeleteDTO.setCondMonMeas(cmRec.condMonMeasure)
            cmDeleteDTO.setMeasureValue(cmRec.measureValue)
            cmDeleteDTO.setMeasureDate(measDate)
            cmDeleteDTO.setMeasureTime(measTime)
            cmDeleteDTO.setRequiredAttributes(reqAttr)

            returnDTO = service.get(SERVICE_NAME_CONDMEASUREMENT).delete(cmDeleteDTO, true)
        } catch (com.mincom.enterpriseservice.exception.EnterpriseServiceOperationException serviceExc) {
            logExceptionService("${SERVICE_NAME_CONDMEASUREMENT}.delete", serviceExc)
        }
        info("end deleteCondMon")
        return returnDTO
    }
