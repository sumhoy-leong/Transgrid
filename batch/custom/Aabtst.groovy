...
//        how_to_invoke_service_and_get_error();
	...
	private void browseMSF891(){
	info("browseMSF891")
	int count = 0
			QueryImpl qMSF891 = new QueryImpl(MSF891Rec.class).
				and(MSF891Key.employeeId.equalTo(empPayrollRec.primaryKey.getEmployeeId())).
				and(MSF891Key.payGroup.equalTo("TG1")).
				and(MSF891Key.trnDate.lessThanEqualTo("20121116")).
				and(MSF891Key.trnDate.greaterThanEqualTo("20121009")).
				and(MSF891Rec.tranApprStatus.equalTo("APPR").
				    or(MSF891Rec.reverseStatus.equalTo("RPLD").
				    or(MSF891Rec.reverseStatus.equalTo("RVSD")))
				)
				edoi.search(qMSF891){MSF891Rec msf891Rec->
				count++
				}
	...
	private void updateWorkOrder(){
		...
		workOrder.setPrefix("00")
		workOrder.setNo("048045")
		String stdjob = "TBOND"
		...
		WorkOrderServiceModifyReplyDTO woReplyDTO = service.get(WORK_ORDER_SERVICE).modify({WorkOrderServiceModifyRequestDTO it->
			...
			it.districtCode = "GRID"
			it.workOrder = workOrder
			it.workOrderDesc = "TEST aja"
			it.stdJobNo = stdjob
		})
    ...
    private void how_to_invoke_service(){
        info("how_to_invoke_service - This command will retrieve the first 20 records ")
        service.get('Table').retrieve({ it.tableType = 'ACTK' }, 20).replyElements.each { println "${it.tableCode} ${it.description}"}
