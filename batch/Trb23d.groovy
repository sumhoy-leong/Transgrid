 ...
 *** Revision History ***
 ***
 *** 28-Oct-13 Yeemun     SC4298302 (Version 0005)
 ...
 ***                       James Stuart said  orders are being displayed for stock items.
 ***                       These should not be included as there is no requesting officer for stock items.
 ***                       so that only non-stock items via purchase requisitions are included.
 */
...
public class ProcessTrb23d extends SuperBatch {
	...
	private void mainProcess(){
		...
		//20131028-Start
		Constraint c5 = MSF220Rec.frelItemInd.equalTo("F")
		Constraint c6 = MSF220Rec.preqItemInd.equalTo("P")
		Constraint c7 = MSF220Rec.servItemInd.equalTo("S")
		
		//20131028-end
		
		def query = new QueryImpl(MSF220Rec.class).and(c1).and(c5.or(c6).or(c7))
				...
				if(msf810rec) {
					...
					if(emailAdress.trim().length() > 0)
					{
						...
						try
						{
							...
							info("*** Cannot find the MSF071 Record with Reference No(${SendEmailRefNo}), Entity Value(${entityVl} and Entity Type (${entityType_PUR})***")
